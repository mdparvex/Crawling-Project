...
async def run_full_crawl(limit=None):
    ...
    async with AsyncClient(base_url=BASE_URL, timeout=20.0, follow_redirects=True) as client:
        ...
        processed_urls = set()
        async for doc in progress_col.find({}, {'_id': 0, 'url': 1}):
        ...
        for link in links:
            ...
            existing = await db.books.find_one({'url': doc['url']})
            await db.books.update_one({'url': doc['url']}, {'$set': doc}, upsert=True)
            await compute_changes_and_log(db, existing, doc)
            # Mark as processed
            await progress_col.insert_one({'url': link})
            processed += 1
