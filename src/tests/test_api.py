...
async def test_list_books_endpoint():
    async with AsyncClient(base_url="http://localhost:8000") as client:
        response = await client.get("/books", headers={"X-API-KEY": API_KEY})
...
async def test_get_book_endpoint():
    async with AsyncClient(base_url="http://localhost:8000") as client:
        ...
        response = await client.get(f"/books/{book_id}", headers={"X-API-KEY": API_KEY})
        assert response.status_code == 200
        book = response.json()
...
async def test_list_changes_endpoint():
    async with AsyncClient(base_url="http://localhost:8000") as client:
        response = await client.get("/changes", headers={"X-API-KEY": API_KEY})
        assert response.status_code == 200
        data = response.json()
